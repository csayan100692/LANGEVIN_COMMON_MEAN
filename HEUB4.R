...
HEUB4=function(x,alpha)
{
  ...
  Y=matrix(c(0.5773503,0.5773503,-0.5773503,0.5773503,0.5773503,-0.5773503,0.5773503,0.5773503,-0.5773503,0.5773503,0.5773503,-0.5773503),nrow = k*(p),ncol = 1,byrow = FALSE)
  #X1=(X-Y)
  #N=sum(sequence_of_n)    #calculations of Q^
  Q=c(sqrt(sequence_of_n[1])*(X[1:3,]-Y[1:3,]),sqrt(sequence_of_n[2])*(X[4:6,]-Y[4:6,]),sqrt(sequence_of_n[3])*(X[7:9,]-Y[7:9,]),sqrt(sequence_of_n[4])*(X[10:12,]-Y[10:12,]))
  ...
  for(h in 1:H)
  {
    ...
    N=sum(sequence_of_n)
    #Q2=sqrt(N)*X3
    Q2=c(sqrt(sequence_of_n[1])*(X2[1:3,]-Y[1:3,]),sqrt(sequence_of_n[2])*(X2[4:6,]-Y[4:6,]),sqrt(sequence_of_n[3])*(X2[7:9,]-Y[7:9,]),sqrt(sequence_of_n[4])*(X2[10:12,]-Y[10:12,]))
